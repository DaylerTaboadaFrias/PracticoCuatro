...
public class ArbolMViasBusqueda<K extends Comparable<K>,V> implements IArbolBinarioBusqueda<K,V> {
    ...
    private void recorridoPostOrdenRecursivo(NodoMVias<K, V> nodoActual, List<K> recorrido) {
        ...
        recorridoPostOrdenRecursivo(nodoActual.getHijo(0),recorrido);
        for(int i = 0; i < nodoActual.cantidadDeDatosNoVacios() ;i++){
            recorridoPostOrdenRecursivo(nodoActual.getHijo(i+1),recorrido);
            recorrido.add(nodoActual.getClave(i));
        }
...
//el predecesor inorden de dicho dato, sin realizar el recorrido en inOrden.
    ...
    //Pregunta 3 : Implementar un método para un árbol de m-vías que retorne cuantos nodos del árbol son padres fuera del nivel n   
    public int cantidadDePadres(int nivel){
        return cantidadDePadres(this.raiz,nivel,0);
    }

    private int cantidadDePadres(NodoMVias<K, V> nodoActual, int nivel, int nivelActual) {
        if(NodoMVias.esNodoVacio(nodoActual)){
            return 0;
        }
        int cantidad = 0 ;
        if( nodoActual.cantidadDeHijosNoVacios() > 0 && nivel != nivelActual)
        { cantidad = cantidad +  1;
        }
        for(int i = 0 ; i < orden ; i ++){
            cantidad = cantidad + cantidadDePadres(nodoActual.getHijo(i),nivel,nivelActual+1);
        }
        return cantidad ;
    }
